...
# ─── Stage 2b: build (TypeScript -> dist/, OSM cache -> data/) ──────────────
...
# data/osm-routes/metro-manila.geojson is generated by `npm run osm:fetch`
# in the build stage. seed.ts resolves it relative to dist/db/, which at
# runtime expands to /app/data/osm-routes/metro-manila.geojson — so this
# layout matters.
